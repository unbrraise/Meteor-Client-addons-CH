...
public class TimerPlus extends Module {
	public TimerPlus() {
		super(MeteorPlus.CATEGORY, "Timer+(某些不是很懂)", "绕过反作弊的改变世界时间流速");
		autoSubscribe = false;
	...
	public final Setting<Boolean> onlyInMove = settingsGroup.add(new BoolSetting.Builder()
		.name("仅在移动时工作")
		.description("Prevent false un charge.(不懂)")
		.defaultValue(true)
		.build()
	);
	...
	private final Setting<Boolean> rechargeOnDisable = sgGeneral.add(new BoolSetting.Builder()
		...
		.description("在禁用模块时，会重新设置计时器的延迟")
		.defaultValue(false)
		.build()
	);
